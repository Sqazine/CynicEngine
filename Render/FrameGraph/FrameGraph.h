...
namespace CynicEngine
{
    ...
    public:
        ...
        RenderTask<DataType> *AddRenderTask(ArgTypes... args)
        {
            ...
            return static_cast<RenderTask<DataType> *>(renderTask);
        }
        ...
        void Compile()
        {
            ...
            while (!unreferencedResources.empty())
            {
                ...
                if (creator->mRefCount == 0 && !creator->IsCullImmune())
                {
                ...
                for (auto writer : unreferencedResource->mWriters)
                {
                    ...
                    if (writeTask->mRefCount == 0 && !writeTask->IsCullImmune())
                    {
            ...
            for (auto &renderTask : mRenderTasks)
            {
                if (renderTask->mRefCount == 0 && !renderTask->IsCullImmune())
                {
                    continue;
                }
